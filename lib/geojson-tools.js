...
function getRideDuration(coordTimesArray, timeType){
  ...
  switch(timeType){
    ...
    case 'moving':
      ...
      coordTimesArray.map((currentTime) => {
        ...
        if(durationDiff.asSeconds() < maxAllowableDurationDiffSeconds){          
          ...
          // console.log("long wait time: ", durationDiff.asSeconds());
        }
...
function getElevationStats(coordsArray){
  ...
  coordsArray.map((coord) => {
    ...
    // maxAllowedGain is the maximum possible number of meters we will allow to be added between two points
    if(Math.abs(delta) < maxAllowedGain){
